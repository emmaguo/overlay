...
class ColorFourViewController: QuestionViewController {
        ...
        if sender.state == UIGestureRecognizerState.Began {
            ...
            if isInView(zoneOneView, shape: blackView) {
                UIView.animateWithDuration(0.1, animations: { () -> Void in
                    ...
                    self.blackView.backgroundColor = blueShade
                })
                ...
                UIView.animateWithDuration(0.1, animations: { () -> Void in
                    ...
                    self.whiteView.backgroundColor = blueTint
                })
    ...
    override func answerIsCorrect() -> Bool {
        let blackInZoneOne = allInArray(blackUnit) { blackUnit in self.isInView(self.zoneOneView, shape: blackUnit) }
        let whiteInOutsideZone = allInArray(whiteUnit) { whiteUnit in self.isInView(self.outsideView, shape: whiteUnit) }

        
        return blackInZoneOne && whiteInOutsideZone
    }
